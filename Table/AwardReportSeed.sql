...
/*
    AUTO-GENERATED — Phase 4.1 Report seed for Award
    Table: [app].[Award]
    ProgramCode: RPT.02.09.05
...
IF NOT EXISTS (SELECT 1 FROM [core].[Programs] WHERE ProgramCode = N'RPT.02.09.05')
BEGIN
    ...
    VALUES
    (NEWID(), N'RPT.02.09.05', N'Award Report', @ReportsMenuId, 2,
     (SELECT ISNULL(MAX(RowIndex), 0) + 1 FROM [core].[Programs] WHERE ParentId = @ReportsMenuId),
     N'RPT', 1, 1, 1, 1, N'app.rpt_Award', @CreatedById, SYSDATETIME(), 1);
END

SELECT @ProgramId = ProgramId FROM [core].[Programs] WHERE ProgramCode = N'RPT.02.09.05';
...
IF NOT EXISTS (SELECT 1 FROM [core].[ReportDefinition]
               WHERE ProgramId = @ProgramId AND DefinitionKey = N'Award' AND CompanyId IS NULL)
BEGIN
    ...
    VALUES
    (NEWID(), @ProgramId, N'Standard', NULL, N'RPT', N'Award', N'FastReport',
     N'Rpt/Standard/Award.frx', N'app.rpt_Award', N'{"paper":"A4","orientation":"Landscape","marginPreset":"Normal","header":{"profile":"Standard","showCompanyLogo":true,"showCompanyName":true,"showOfficeName":false,"showPrintedAt":false}}', 0, 0, N'PerRun', N'AW',
...
BEGIN
    ...
    SET RendererType = N'FastReport',
        FilePath = N'Rpt/Standard/Award.frx',
        StoreProcedureName = N'app.rpt_Award',
...
PRINT 'Award report + parameter mapping seeded.';
GO